...
function App() {
  ...
  const onContinentChange = (e) => {
    ...
    setGeoName(geoName);
    fetch(
      `https://api.teleport.org/api/continents/geonames%3A${geoName}/urban_areas/`
    )
  ...
  return (
    <div className="App">
      <div className="header">
        ...
        <h2
          className={
            !visibility ? "select_a_continent" : "select_a_continent--hidden"
          }
        >
        ...
        <FormControl>
          ...
          >
            {continents.map((continent) => (
              <MenuItem value={continent.geonames}>{continent.name}</MenuItem>
            ))}
          </Select>
        </FormControl>
      </div>
      ...
      <div
        className={!visibility ? "card__container" : "card__container--hidden"}
      ></div>
