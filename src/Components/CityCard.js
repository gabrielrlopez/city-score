...
function CityCard({
  ...
  const click = (e) => {
    ...
    const link = scoreLinks.find((link) =>
      link.includes(
        city.replace(/[,.]/g, "").toLowerCase().split(" ").join("-")
      )
    );
    fetch(link)
      .then((response) => response.json())
  ...
  return (
    <>
      <h2
        className={cityCardHeader ? "select_a_city" : "select_a_city--hidden"}
      >
        Select a city to view its score out of ten on several different
        categories.
      </h2>
      ...
      <div className={clicked ? "city__cards--hidden" : "city__cards"}>
        {cities.map((city, i) => (
          <Card className="city__card" onClick={click}>
            <CardActionArea>
              <CardMedia
      ...
      <h2 className={scoreCardHeader ? "rainbow" : "rainbow--hidden"}>
        Here's a rainbow in case you didn't see one today :)
      </h2>

      {/**Score cards */}
      <div
        className={
          hiddenScores || changedContinent
            ? "score__cards--hidden"
            : "score__cards"
        }
      >
